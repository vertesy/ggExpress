...
data("ToothGrowth"); ToothLen.by.Dose <- ToothGrowth[ ,c('dose', 'len')]; qviolin(ToothLen.by.Dose)
...
ggviolin(df, x = "dose", y = "len", color = "dose", width = 0.8)
...
x <- ggviolin(data = combined.obj@meta.data, "is.Stressed", 'percent.mito', fill = "is.Stressed", outlier.shape = NA, ylim = c(0, 0.06))
...
qviolin(dfm.is.Stressed.percent.mito
         , stat.method = "wilcox.test", stat.label.x = .5)
